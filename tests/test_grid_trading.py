...
def make_candles(rows):
    rows = list(rows)
    times = pd.date_range('2024-01-01', periods=len(rows), freq='min', tz='UTC')
    return pd.DataFrame({
        'open_time': times,
...
class TestGridTradingEngine(unittest.TestCase):
    def setUp(self):
        # 3 intervals: 100->110, 110->120, 120->130; 100 USDT/grid.
    ...
    def test_excel_template_checkpoint(self):
        grid = build_excel_grid_table(3000.0, 8987.0, 1987.0, 70.0, 0.001, 0.001)
        first = grid.iloc[0]
        ...
        self.assertAlmostEqual(first['profit'], 0.1750644398340242, places=12)

    def test_grid_count_and_capital_per_level(self):
        self.assertEqual(len(self.grid), 3)
        self.assertTrue(np.allclose(self.grid['capital_per_level'], 100.0))

    def test_fee_formula_matches_excel_logic(self):
        row = self.grid.loc[self.grid['buy_price'].eq(120.0)].iloc[0]
        ...
        self.assertAlmostEqual(row['gross_base_amount'], gross_base, places=12)
        self.assertAlmostEqual(row['buy_fee_base'], expected_buy_fee_base, places=12)
        self.assertAlmostEqual(row['base_amount'], expected_base, places=12)
        self.assertAlmostEqual(row['sell_fee_quote'], expected_sell_fee, places=12)
        self.assertAlmostEqual(row['net_sell'], expected_net_sell, places=12)

    def test_upward_move_does_not_trigger_buy(self):
        result = run_grid_backtest(make_candles([(115, 125, 115, 122)]), self.grid, 300.0)
    ...
    def test_downward_cross_triggers_buy(self):
        result = run_grid_backtest(make_candles([(125, 126, 115, 118)]), self.grid, 300.0)
    ...
    def test_new_buy_cannot_sell_in_same_candle(self):
        result = run_grid_backtest(make_candles([(125, 135, 115, 120)]), self.grid, 300.0)
    ...
    def test_buy_then_sell_on_later_candle(self):
        data = make_candles([(125, 126, 115, 118), (120, 131, 120, 130)])
        ...
        ref = self.grid.loc[self.grid['buy_price'].eq(120.0)].iloc[0]
        self.assertAlmostEqual(result['summary']['realized_profit'], ref['profit'], places=12)

    def test_multiple_grid_levels_fill_on_downward_cross(self):
        result = run_grid_backtest(make_candles([(125, 126, 95, 100)]), self.grid, 300.0)
        buys = result['trade_log'].loc[result['trade_log']['side'].eq('BUY')]
        self.assertEqual(len(buys), 3)
        self.assertEqual(set(buys['price'].tolist()), {100.0, 110.0, 120.0})
        self.assertAlmostEqual(result['summary']['final_cash'], 0.0, places=10)

    def test_holding_grid_cannot_buy_twice(self):
        data = make_candles([(125, 126, 115, 118), (125, 126, 115, 118)])
        result = run_grid_backtest(data, self.grid, 300.0)
        self.assertEqual(len(result['trade_log']), 1)
        self.assertEqual(result['trade_log'].iloc[0]['side'], 'BUY')

    def test_sold_grid_cannot_rebuy_in_same_candle(self):
        data = make_candles([(125, 126, 115, 118), (125, 131, 115, 120)])
        result = run_grid_backtest(data, self.grid, 300.0)
        self.assertEqual(result['trade_log']['side'].tolist(), ['BUY', 'SELL'])
        self.assertEqual(result['summary']['open_positions'], 0)

    def test_insufficient_cash_prevents_overbuying(self):
        result = run_grid_backtest(make_candles([(125, 126, 95, 100)]), self.grid, 100.0)
        buys = result['trade_log'].loc[result['trade_log']['side'].eq('BUY')]
    ...
    def test_grid_cashflow_matches_excel_profit(self):
        data = make_candles([(125, 126, 115, 118), (120, 131, 120, 130)])
        result = run_grid_backtest(data, self.grid, 300.0)
        log = result['trade_log']
        ref = self.grid.loc[self.grid['buy_price'].eq(120.0)].iloc[0]
        self.assertAlmostEqual(log.iloc[0]['grid_cashflow'], 0.0, places=12)
        self.assertAlmostEqual(log.iloc[1]['grid_cashflow'], ref['profit'], places=12)
        self.assertAlmostEqual(
            log['grid_cashflow'].sum(),
            result['summary']['realized_profit'],
            places=12,
        )
        completed = result['completed_trades'].iloc[0]
        self.assertAlmostEqual(completed['actual_earn'] - completed['cost'], completed['grid_cashflow'], places=12)

    def test_equity_identity(self):
        data = make_candles([(125, 126, 115, 118), (120, 125, 117, 123)])
        result = run_grid_backtest(data, self.grid, 300.0)
